...
function ShiftTracker({ session, onShowPrivacy }) {
  ...
  const nextPayday = useMemo(() => {
    const paydays = enriched.map((s) => s.payday).filter(Boolean).filter((d) => d >= today).sort();
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.paper, fontFamily: FONT_BODY }}>
      ...
      <div style={{ background: COLORS.navy, padding: "1.5rem 1.25rem 2.5rem" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            <div>
              <p style={{ color: COLORS.amber, fontFamily: FONT_MONO, fontSize: 11, letterSpacing: 2, margin: 0 }}>
                ZERO CONTRACT
              </p>
              {firstName && (
                <p style={{ color: COLORS.offwhite, opacity: 0.55, fontFamily: FONT_BODY, fontSize: 13, margin: "2px 0 0" }}>
                  {firstName}'s ledger
                </p>
              )}
            </div>
            <button onClick={() => setShowSettings(true)} style={iconGhostButtonStyle} aria-label="Settings">
          ...
          {taxEstimate && (
            <button
              ...
              style={{ marginTop: 10, border: "none", background: "rgba(255,255,255,0.08)", borderRadius: 4, padding: "6px 12px", color: COLORS.offwhite, fontFamily: FONT_MONO, fontSize: 11, letterSpacing: 1, cursor: "pointer", opacity: 0.8 }}
            >
          ...
          {taxEstimate && showTax && (
            <div style={{ marginTop: 12, background: "rgba(255,255,255,0.07)", borderRadius: 6, padding: "14px 16px" }}>
              <p style={{ color: COLORS.amber, fontFamily: FONT_MONO, fontSize: 10, letterSpacing: 2, margin: "0 0 10px" }}>
          ...
          {profile?.tax_region === "skip" && (
            <button
              ...
              style={{ marginTop: 10, border: "1px dashed rgba(255,255,255,0.25)", background: "transparent", borderRadius: 4, padding: "6px 12px", color: COLORS.offwhite, fontFamily: FONT_MONO, fontSize: 11, letterSpacing: 1, cursor: "pointer", opacity: 0.7 }}
            >
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "1.75rem 1.25rem 4rem" }}>
        <button onClick={() => { resetForm(); setShowForm(true); }} style={addTabStyle}>
          <Plus size={15} />
          ADD SHIFT
        </button>
        ...
        <Ledger
          ...
          shifts={[...past].reverse()}
          emptyText="No shifts logged yet."
          onEdit={handleEdit}
          onDelete={handleDelete}
          onTogglePaid={togglePaid}
        />
...
function TaxRow({ label, value, highlight }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
      <span style={{ color: COLORS.offwhite, opacity: highlight ? 1 : 0.65, fontFamily: FONT_MONO, fontSize: 12, letterSpacing: 0.5 }}>
        {label}
      </span>
      <span style={{ color: highlight ? COLORS.amber : COLORS.offwhite, opacity: highlight ? 1 : 0.8, fontFamily: FONT_MONO, fontSize: 12, fontWeight: highlight ? 600 : 400 }}>
        {formatMoney(value)}
      </span>
    </div>
  );
...
function ShiftForm({ form, setForm, onSave, onClose, visible, saving, editingId }) {
  const hours = calcHours(form.start, form.end);
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 6px 20px rgba(21,32,59,0.15)",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <p style={{ fontFamily: FONT_MONO, fontSize: 11, letterSpacing: 1, color: COLORS.inkSoft, margin: 0 }}>
    ...
    <div style={{ display: "flex", alignItems: "stretch", background: "white", borderRadius: 4, marginBottom: 7, boxShadow: "0 1px 3px rgba(21,32,59,0.08)", overflow: "hidden" }}>
      ...
      <button
        ...
        style={{
          ...
          alignItems: "center",
          cursor: "pointer",
          transform: tearing ? "scale(0.85) rotate(-4deg)" : "scale(1) rotate(0deg)",
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(21,32,59,0.85)", zIndex: 50, display: "flex", justifyContent: "center", alignItems: "flex-start", overflowY: "auto", padding: "2rem 1rem", fontFamily: FONT_BODY }}>
      <div style={{ background: COLORS.paper, borderRadius: 8, maxWidth: 420, width: "100%", padding: "1.75rem" }}>
        ...
        <div style={{ marginBottom: 22 }}>
          <label style={fieldLabelStyle}>Other annual income (GBP) &mdash; optional</label>
          ...
          <p style={{ fontSize: 12, color: COLORS.inkSoft, margin: "6px 0 0", lineHeight: 1.4 }}>
            If you have other jobs or income sources, add the annual total here so the tax estimate accounts for your full picture.
          </p>
        </div>
